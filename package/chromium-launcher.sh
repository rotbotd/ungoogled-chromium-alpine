...
if [ "$(id -u)" -eq 0 ] &&
  [ "$(stat -c %u -L "${XDG_CONFIG_HOME:-${HOME}}")" -eq 0 ]; then
...
export CHROME_VERSION_EXTRA="Alpine Linux (ungoogled)"

# The configured string must expand into separate command-line arguments.
